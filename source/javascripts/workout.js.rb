...
myWeb = "'Workout' "
essence = "helps me to practice Coding."
...
class Editor
  ...
  def mark_ok(line_from, line_to)
    `#@native.markText({line: line_from, ch: 0}, {line: line_to, ch: 99}, {className: "workout-output-green"})`
  end

  def mark_error(line_from, line_to)
    `#@native.markText({line: line_from, ch: 0}, {line: line_to, ch: 99}, {className: "workout-output-red"})`
  end
end
...
class Workout
  ...
  def initialize
    ...
    # Stop if this is not a workout enabled page
    return if Element.find('#workout-title').empty?
    ...
    # Is this the workout page ? If so -> exit
    if Element.find('#workout-title').html.match(/workout/i)
  ...
  def get_language
    ...
    language = get_cookie('workout_nl_language')
    ...
    if language.empty?
      ...
      set_cookie('workout_nl_language', language)
    end

    # Update language select list
    Element.find('#workout-lang-select').value = language
  ...
  def get_content_from_server(language)
    HTTP.get("workout_#{language}.json") do |response|
      # Handle ajax reponse
      if response.status_code == 200
        # Store items
        @items = {}
        response.json.each do |k, v|
          @items[k.to_i] = WorkoutItem.new(k, v)
        end
        @loaded = true
  ...
  def create_index
    ...
    Element.find('#workout-index').html = index
    Element.find('#workout-index').on(:change) { do_goto }
  end
  ...
  def switch_to_last_used
    last_step = get_cookie('workout_nl_step').to_i
  ...
  def do_goto
    return if @updating
    new_value = Element.find('#workout-index').value.to_i
  ...
  def get_code_fragment(str)
    # Let jQuery find the first code fragment in workout-content
    code = Element.find('#workout-content code').html.strip
  ...
  def update_screen(item)
    return if !item
    @updating = true
    Element.find('#workout-title').html   = item.title
    Element.find('#workout-content').html = item.text
    Element.find('#workout-answer').html  = item.answer
    ...
    step = item.step.to_s
    Element.find("#workout-index").value  = step

    # Set session cookie to store progress
    set_cookie('workout_nl_step', step)
...
def start_workout
  ...
  def $stdout.puts(*strs)
    strs.each { |str| Workout.instance.print_to_output str}
    strs.last
  end
